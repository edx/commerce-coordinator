...
.PHONY: help clean clean_tox piptools requirements ci_requirements \
        ...
        open-devstack pkg-devstack detect_changed_source_translations \
...
dev_requirements: clean_tox ## sync to requirements for local development
...
isort_check: ## check that isort has been run
	isort --check-only commerce_coordinator/
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it commerce_coordinator.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it commerce_coordinator.db mysql -u root commerce_coordinator
