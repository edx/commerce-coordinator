...
def calculate_discount(order: Order) -> TypedMoney:
    ...
    total_discount_cent_amount = discount_on_total_price.cent_amount + discount_on_line_items
...
def extract_ct_order_information_for_braze_canvas(customer: Customer, order: Order):
    ...
    total_discount = calculate_discount(order)
    # calculate subtotal by adding discount back if any discount is applied.
    subtotal = (((order.total_price.cent_amount +
                  total_discount.cent_amount))
                if total_discount else order.total_price.cent_amount)
