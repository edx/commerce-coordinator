...
class MobileStandalonePriceChangeView(SingleInvocationAPIView):
    """Handles price change messages from AWS EventBridge for mobile standalone products"""
    ...
    def post(self, request):
        ...
        logger.info(
            ...
            f'StandAlone Price Changed with details: {input_data}'
        )
...
class MobileCourseVariantAddView(SingleInvocationAPIView):
    ...
    def post(self, request):
        ...
        logger.info(
            f'[CT-{tag}] Message received from commercetools '
            f'Course Variant Added Price Changed with details: {input_data}'
        )
