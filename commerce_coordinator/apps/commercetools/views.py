...
class OrderReturnedView(APIView):
    ...
    def post(self, request):
        ...
        for line_item in get_edx_items(order):
            ...
            product = {
                ...
                'price': line_item.price.value.cent_amount / pow(
                    10, line_item.price.value.fraction_digits
                    if hasattr(line_item.price.value, 'fraction_digits')
                    else 2
                ),
                'quantity': line_item.quantity,
                'category': self._get_item_category(line_item),
                'image_url': line_item.variant.images[0].url if line_item.variant.images else None,
                'brand': self._get_item_brand(line_item)
            }
    ...
    @staticmethod
    def _get_item_category(line_item):  # pragma no cover
        """Utility to get line item's category name."""
        category = None
        for attribute in line_item.variant.attributes:
            if attribute.name == 'primarySubjectArea' and hasattr(attribute, 'value'):
                if isinstance(attribute.value, dict):
                    category = attribute.value.get('label', None)
    ...
    @staticmethod
    def _get_item_brand(line_item):  # pragma no cover
        """Utility to get line item's brand name."""
        brand = None
        for attribute in line_item.variant.attributes:
            if attribute.name == 'brand-text' and hasattr(attribute, 'value'):
                if isinstance(attribute.value, dict):
                    brand = attribute.value.get('label', None)
                elif isinstance(attribute.value, str):
    ...
    @staticmethod
    def _prepare_segment_event_properties(order):  # pragma no cover
        return {
            'track_plan_id': 19,
            'trigger_source': 'server-side',
            'order_id': order.id,
            'checkout_id': order.cart.id,
            'return_id': '',  # TODO: Set CT return ID here.
            'total': order.taxed_price.total_gross.cent_amount / pow(
                10, order.taxed_price.total_gross.fraction_digits
                if hasattr(order.taxed_price.total_gross, 'fraction_digits')
                else 2
            ),
            'currency': order.taxed_price.total_gross.currency_code,
            'tax': order.taxed_price.total_tax.cent_amount / pow(
                10, order.taxed_price.total_tax.fraction_digits
                if hasattr(order.taxed_price.total_tax, 'fraction_digits')
                else 2
            ),
            ...
            'discount': order.discount_on_total_price.discounted_amount.cent_amount / pow(
                10, order.discount_on_total_price.discounted_amount.fraction_digits
                if hasattr(order.discount_on_total_price.discounted_amount, 'fraction_digits')
                else 2
            ) if order.discount_on_total_price else 0,
