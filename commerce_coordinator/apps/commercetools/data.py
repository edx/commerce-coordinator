...
from commerce_coordinator.apps.commercetools.utils import calculate_discount
...
def order_from_commercetools(order: CTOrder, customer: CTCustomer) -> LegacyOrder:
    ...
    discounted_amount = calculate_discount(order)
    ...
    return LegacyOrder(
        ...
        total_before_discounts_incl_tax=typed_money_to_string(
            typed_money_add(
                typed_money_add(
                    order.total_price,
                    discounted_amount if discounted_amount else None
                ),
