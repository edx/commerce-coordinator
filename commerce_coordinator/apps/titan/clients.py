...
class TitanAPIClient(Client):
    ...
    def create_payment(
        ...
        Args:
            ...
            amount(int): Optional. Payment amount to apply to order.
            payment_date(POSIX date): Optional. Payment date to save against the payment.
            source(dict): Optional. Payment source details to be saved. It should have 'ccType',
                                    'gatewayCustomerProfileId', 'gatewayPaymentProfileId', 'lastDigits', 'month',
                                     'name' and 'year' keys in 'str' format.
        """
        payload_attributes = {
            'orderUuid': order_uuid,
            'responseCode': response_code,
