...
class GetPaymentProcessorTests(TestCase):
    ...
    def test_valid_android_iap(self, MockProcessor):
        ...
        result = get_payment_info_from_purchase_token(self.request_data, self.cart)

        self.assertEqual(result["status_code"], status.HTTP_200_OK)
        self.assertEqual(result["response"], {"success": True})
        mock_processor_instance.validate_iap.assert_called_once_with(self.request_data, self.cart)
