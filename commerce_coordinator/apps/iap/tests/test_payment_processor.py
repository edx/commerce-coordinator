...
class TestIAPPaymentProcessor(unittest.TestCase):
    ...
    def test_android_validation_success(self, MockGooglePlayValidator):
        ...
        result = self.processor.validate_iap(request_data, cart_id='cart_123')
    ...
    def test_android_validation_failure_raises_validation_error(self, MockGooglePlayValidator):
        ...
        with self.assertRaises(ValidationError) as cm:
            self.processor.validate_iap(request_data, cart_id='cart_123')
    ...
    def test_android_handle_android_validation_errors(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            self.processor._handle_android_validation(
                {'raw_response': {}}, cart_id='cart_123')
        self.assertIn("missing transaction ID", str(cm.exception))

        # Test is_canceled True raises UserCancelled
        with self.assertRaises(UserCancelled) as cm:
            self.processor._handle_android_validation(
                {'raw_response': {'orderId': 'txn_123'}, 'is_canceled': True}, cart_id='cart_123')
        self.assertIn("cancelled", str(cm.exception))

        # Test is_expired True raises PaymentError
        with self.assertRaises(PaymentError) as cm:
            self.processor._handle_android_validation(
                {'raw_response': {'orderId': 'txn_123'}, 'is_expired': True}, cart_id='cart_123')
    ...
    def test_android_redundant_payment_error(self, MockGooglePlayValidator):
        ...
        with self.assertRaises(RedundantPaymentError) as cm:
            self.processor.validate_iap(
                {'purchase_token': 'token', 'payment_processor': 'android-iap'}, cart_id='cart_123')

        self.assertIn("Redundant payment", str(cm.exception))
    ...
    def test_ios_handle_ios_validation_errors(self):
        ...
        self.assertIn("No matching iOS IAP purchase found", str(cm.exception))

        # No matching purchase with wrong SKU
        with self.assertRaises(ValidationError) as cm:
        ...
        # Cancellation reason raises UserCancelled
        with self.assertRaises(UserCancelled) as cm:
            self.processor._handle_ios_validation(
                {
                    'receipt': {
        ...
        self.assertIn("iOS payment is cancelled", str(cm.exception))
    ...
    def test_missing_payment_processor_raises_validation_error(self):
        request_data = {
        ...
        with self.assertRaises(ValidationError) as cm:
            self.processor.validate_iap(request_data, cart_id='cart_123')
        self.assertIn("Missing 'payment_processor'", str(cm.exception))
    ...
    def test_unsupported_payment_processor_raises_validation_error(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            self.processor.validate_iap(request_data, cart_id='cart_123')
        self.assertIn("Unsupported payment_processor", str(cm.exception))
