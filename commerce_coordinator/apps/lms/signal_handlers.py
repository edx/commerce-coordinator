...
@log_receiver(logger)
def fulfill_order_placed_send_entitlement(**kwargs):
    """
    Fulfill the order placed with a Celery task to LMS to add entitlements of a program for a user.
    """
    async_result = fulfill_order_placed_send_entitlement_task.delay(
        course_id=kwargs['course_id'],
        course_mode=kwargs['course_mode'],
        edx_lms_user_id=kwargs['edx_lms_user_id'],
        email_opt_in=kwargs['email_opt_in'],
